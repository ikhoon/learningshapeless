...
object Ch01_HLists extends App {
  ...
  def ex_prependAndAppend = "Issac" +: ex_newton :+ "Calculus"
  ...
  def ex_tuple_append: (String, Int, Country, String) = ex_tuple :+ "Calculus"
  ...
  def ex_from_tuple: String :: Int :: Country :: String :: HNil = ex_tuple_append.productElements
  ...
  * A */
  ...
  object NameCapPloy extends DefaultIdentityMapping {
    implicit def allCaps = at[String](_.toUpperCase)
  }
  def ex_poly = ex_newton.map(NameCapPloy)
  ...
  * `updateAtWith` returns the old value, and the updated HList in a tuple */
  def ex_to_firstname_by_index: (String, HList) = ex_newton.updateAtWith(0)(name => Data.scientistsFirstNames.get(name))
